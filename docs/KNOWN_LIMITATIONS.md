...
- **Two-way SMS (student texts to request time, parent replies YES/NO) is off until a real Twilio number is configured** — see `docs/DEPLOYMENT.md` section 4.6. Once on, matching an inbound text to a student or parent is by exact phone number after light US-centric normalization (`services/api/app/phone.py`); it isn't a full libphonenumber-grade implementation, and there's no verification step confirming a parent/student actually owns the number they typed into the dashboard before texts start being trusted from it.
