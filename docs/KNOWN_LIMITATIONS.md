...
- **Extension-request approve/deny emails need a real email provider to actually deliver** — see `docs/DEPLOYMENT.md` section 4.6. The links themselves are always generated; with the default `console` email provider they only show up in the notification-worker's logs instead of a real inbox. There is no SMS/texting feature in this app — that was tried and removed in favor of the email approach, since it requires no phone number or carrier verification.
